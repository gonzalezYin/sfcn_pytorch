...
# ================ data setting ====================
parser.add_argument('--dataset_name', metavar='DATASET', default='BSD500', choices=dataset_names,
...
parser.add_argument('--save_path', default='./ckpt_log',
                    help='path to save ckpt')

parser.add_argument('--train_img_height', '-t_img_h', default=208, type=int,
                    help='img height')
parser.add_argument('--train_img_width', '-t_img_w', default=208, type=int,
                    help='img width')
parser.add_argument('--val_img_height', '-v_img_h', default=320, type=int,
                    help='img height_must be 16*n')  #
parser.add_argument('--val_img_width', '-v_img_w', default=320, type=int,
                    help='img width must be 16*n')

# ======== weight setting ================
parser.add_argument('--epoch_end', default=300000, type=int, metavar='N',
                    help='number of total epochs, make it big enough to follow the iteration maximum')
parser.add_argument('--epoch_begin', default=0, type=int, metavar='N',
...
parser.add_argument('--lr', '--learning_rate', default=1e-6, type=float, metavar='LR',
                    help='initial learning rate')
parser.add_argument('--beta', default=0.999, type=float, metavar='M',
                    help='beta parameter for adam')
parser.add_argument('--weight_decay', '--wd', default=4e-4, type=float, metavar='W',
                    help='weight decay')
parser.add_argument('--bias_decay', default=0, type=float, metavar='B',
                    help='bias decay, we never use it')
parser.add_argument('--additional_step', default=100000,
...
parser.add_argument('--pos_weight', '-p_w', default=0.003, type=float,
                    help='weight of the pos term')

# ================= train setting ===================
parser.add_argument('--gpu', default='0', type=str,
                    help='gpu id')
parser.add_argument('--pretrained_model', default='./pretrained_ckpt/SpixelNet_bsd_ckpt.tar',
                    help='path to pretrained model')
parser.add_argument('-j', '--workers', default=4, type=int, metavar='N',
                    help='number of data loading workers')
parser.add_argument('--milestones', default=[200000], metavar='N', nargs='*',
                    help='epochs at which learning rate is divided by 2')
parser.add_argument('--solver', default='adam', choices=['adam', 'sgd'],
                    help='optimizer algorithms, we use adam')

# ================= writer setting ===================
parser.add_argument('--writer_dir', default='./writer_record', type=str,
                    help='dir to store the tensorboardX writer')
parser.add_argument('--print_freq', '-p', default=10, type=int,
                    help='print frequency (step)')
parser.add_argument('--record_freq', '-rf', default=5, type=int,
...
def main():
    ...
    # # ============= savor setting ===================
    # res_path = '{}_{}_{}epochs{}_b{}_lr{}_posW{}'.format(
    #     args.arch,
    #     args.optim,
    #     args.epochs,
    #     '_epoch_size' + str(args.epoch_size) if args.epoch_size > 0 else '',
    #     args.batch_size,
    #     args.lr,
    #     args.pos_weight,
    # )
    ...
    save_path = os.path.abspath(args.save_path) + '/' + os.path.join(args.dataset_name)
    train_writer = SummaryWriter(os.path.join(args.writer_dir, 'train'))
    val_writer = SummaryWriter(os.path.join(args.writer_dir, 'val'))
    ...
    # =============== load data ==================================================
    print("=> load img {} from '{}'".format(args.dataset_name, args.dump_root))
    train_set, val_set = datasets.__dict__[args.dataset_name](
        args.dump_root,
